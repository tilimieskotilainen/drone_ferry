...
def read_compass():
    ...
    while True:
        ...
        #Heading before making the magnetic inclination adjustment is saved as variable head
        if y == 0:
            if x < 0:
                head = -90
            else:
        ...
        #In situations where y is positive, there are two scenarios to give us the type of heading we want (up to -180 for left and up to 180 for right)
        elif y > 0:
            if x > 0:
        ...
        #Heading is calculated by adding the magnetic inclination in Espoo to the result from our raw trigonometry
